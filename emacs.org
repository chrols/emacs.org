...
#+BEGIN_SRC emacs-lisp
  (let ((preferred-font "PragmataPro-11"))
...
I do not want any trailing whitespace. That said CM code is littered
with it. Having it enabled will result in noisy commits.
#+BEGIN_SRC emacs-lisp
  (add-hook
   'before-save-hook
   (lambda ()
     (unless (string= (file-name-extension buffer-file-name) "cm")

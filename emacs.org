...
#+BEGIN_SRC emacs-lisp
  ...
  (defun configura-buffer-p ()
    (or (string= (file-name-extension buffer-file-name) "cm")
...
Add [[https://melpa.org/][MELPA]] and [[https://orgmode.org/][org-mode]] archives to ~package-archives~.
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; In case ELPA or MELPA is down the following mirror can be used:
...
** Work setup
Load work specific stuff from a separate file.
#+BEGIN_SRC emacs-lisp
  (if (work-computer-p)
      (org-babel-load-file "~/org/emacs/work.org"))
#+END_SRC
...
I do not want any trailing whitespace. That said, Configura code is littered
with it. Having it enabled will result in noisy commits.
#+BEGIN_SRC emacs-lisp
  (add-hook
   'before-save-hook
   (lambda ()
     (unless (configura-buffer-p)
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "H-<f5>")  '(lambda () (interactive) (find-file
                                                        (if (work-computer-p)
                                                            "C:/projects/emacs-org/emacs.org"
...
#+END_SRC
** Flycheck
Use flycheck unless we are in a Configura context.
...
#+BEGIN_SRC emacs-lisp
  (use-package clang-format
      ...
      (add-hook 'before-save-hook (lambda ()
                                    (when (and (eq major-mode 'c++-mode) (not (configura-buffer-p)))
                                      (clang-format-buffer))))
...
#+BEGIN_SRC emacs-lisp
  (when (work-computer-p)
    (setq python-shell-interpreter "C:/Users/chrol/AppData/Local/Programs/Python/Python37-32/python.exe")
